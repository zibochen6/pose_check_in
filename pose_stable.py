...
openai_client = OpenAI(
    ...
    timeout=API_TIMEOUT  # 🔧 修复1: 添加超时设置
)
...
def inference_worker():
    """后台推理线程"""
    global last_results, program_running
    while program_running:
        try:
            frame = inference_queue.get(timeout=1)  # 🔧 修复: 添加超时避免阻塞
            ...
            if not isinstance(e, TimeoutError):  # 忽略超时异常
                print(f"推理错误: {e}")
...
def generate_anime_image_async(frame, person_bbox):
    ...
    🔧 修复: 改进异常处理和状态管理
    """
    global generated_image, is_generating, display_state
    ...
    try:
        ...
        threshold = getattr(globals().get('config', {}), 'SEGMENTATION_THRESHOLD', 0.7)  # 🔧 修复: 使用默认值
